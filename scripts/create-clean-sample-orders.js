// Create 5 clean sample orders with valid status values
const fetch = globalThis.fetch || require('node-fetch');

const BASE_URL = 'http://localhost:9002';

// Clean sample order data with only valid status values
const sampleOrders = [
  {
...
async function createSampleOrders() {
  console.log('🎯 Creating 5 Sample Orders with Valid Status Values...\n');
  ...
  try {
    ...
    for (let i = 0; i < sampleOrders.length; i++) {
      ...
      console.log(`📝 Creating Order ${i + 1}: ${order.orderReference} (${order.status})`);
