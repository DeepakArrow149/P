...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    try {
      ...
      const { id } = await params;
      const buyer = await MockBuyerService.findById(id);
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const useMockData = !(await isDatabaseAvailable());
    ...
    if (useMockData) {
      console.log('Using mock data for buyer update');
      updatedBuyer = await MockBuyerService.update(id, validatedData);
    } else {
      ...
      // Update buyer
      updatedBuyer = await BuyerRepository.update(id, validatedData);
    }
    ...
    try {
      console.log('Database failed, falling back to mock data for update');
      const { id } = await params;
      const body = await request.json();
      const validatedData = buyerSchema.parse(body);
      const updatedBuyer = await MockBuyerService.update(id, validatedData);
      ...
      if (mockError instanceof Error && (mockError.message === 'Buyer not found' || mockError.message.includes('already exists'))) {
        const status = mockError.message === 'Buyer not found' ? 404 : 409;
        return NextResponse.json(
          { 
            success: false, 
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    let deleted;
    if (useMockData) {
      console.log('Using mock data for buyer deletion');
      deleted = await MockBuyerService.delete(id);
    } else {
      deleted = await BuyerRepository.delete(id);
    }
    ...
    console.error(`DELETE /api/buyers/${(await params).id} error:`, error);
      // Fallback to mock data
    try {
      console.log('Database failed, falling back to mock data for deletion');
      const { id } = await params;
      const deleted = await MockBuyerService.delete(id);
      ...
      if (mockError instanceof Error && mockError.message === 'Buyer not found') {
        return NextResponse.json(
          { 
            success: false, 
            error: 'Buyer not found' 
